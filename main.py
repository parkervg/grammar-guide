...
import outlines
from grammar_guide.minEarley.parser import EarleyParser
from grammar_guide._logger import logger
from grammar_guide.typedefs import GrammarGuideOutput, Correction
...
def forward_pass_no_sample(input_ids, past_key_values: Optional[DynamicCache] = None):
...
def generate(
    ...
    prompt: str,
    draft_model: Optional[outlines.models.LogitsGenerator],
    ...
    start = time.time()
    past_key_values = forward_pass_no_sample(
        tokens[: len(prompt_input_ids)].unsqueeze(0)
    )
    ...
    while num_correction_left > 0 and ret_prediction is None:
        ...
        # For example, if we just forgot a semicolon at the end of a JavaScript line
            ...
            past_key_values = forward_pass_no_sample(
                selected_candidate_ids, past_key_values=past_key_values
            )
